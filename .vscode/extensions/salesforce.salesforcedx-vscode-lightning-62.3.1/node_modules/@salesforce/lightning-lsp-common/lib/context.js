...
async function readSfdxProjectConfig(root) {
    try {
        return JSON.parse(await fs.readFile((0, shared_1.getSfdxProjectFile)(root), 'utf8'));
    }
    catch (e) {
        throw new Error(`Sfdx project file seems invalid. Unable to parse ${(0, shared_1.getSfdxProjectFile)(root)}. ${e.message}`);
    }
}
...
async function findCoreESLint() {
    ...
    const eslintToolDir = path.join((0, os_1.homedir)(), 'tools', 'eslint-tool');
    ...
    for (const file of dirs) {
        ...
        if ((await fs.stat(subdir)).isDirectory()) {
            if (!highestVersion || (0, semver_1.lt)(highestVersion, file)) {
                highestVersion = file;
            }
        }
    }
...
class WorkspaceContext {
    ...
    constructor(workspaceRoots) {
        ...
        this.type = (0, shared_1.detectWorkspaceType)(this.workspaceRoots);
    ...
    async updateCoreSettings() {
        ...
        const variableMap = Object.assign({ eslint_node_path: await findCoreESLint() }, configBlt);
    ...
    async readConfig() {
        ...
        else if (this.type === shared_1.WorkspaceType.CORE_ALL) {
            userProperties = path.join(this.workspaceRoots[0], 'build', 'user.properties');
        }
        let properties = {};
        try {
            const userPropertiesContent = await fs.readFile(userProperties, 'utf8');
            properties = (0, properties_1.parse)(userPropertiesContent);
        }
        catch (error) {
    ...
    processTemplate(templateString, variableMap) {
        ...
        const compiled = (0, lodash_template_1.default)(templateString);
    ...
    async updateForceIgnoreFile(ignoreFile, hasTsEnabled) {
    ...
    async updateNamespaceRootTypeCache() {
        this.findNamespaceRootsUsingTypeCache = utils.memoize(this.findNamespaceRootsUsingType.bind(this));
    }
}
